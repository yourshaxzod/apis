...
const { getMusicDown } = require('./utils/down')
...
app.get('/youtube/', async (req, res) => {
    const mediaUrl = req.query.url;
    const videoRegex = /https:\/\/(?:youtu\.be\/([a-zA-Z0-9_-]+)|(?:www\.)?youtube\.com\/shorts\/([a-zA-Z0-9_-]+))(?:\?[^ ]*)?/;
    ...
    try {
        ...
        if (match) {
            result = await getYoutube(mediaUrl);
        } else {
            result = {
                ok: false,
